...
class ISCTrainer:
    ...
    def train_epoch(self):
        ...
        for i, (images, word_inputs, word_targets, lengths, ids, labels) in enumerate(self.data_loader):
            ...
            if self.train:
                ...
                loss = self.train_step(images, word_targets, labels, lengths)
                ...
                word_targets= word_targets.to(self.device)
                labels = labels.to(self.device)
                score = self.eval_step(images, word_targets, labels, lengths)
                result.append(score)
    ...
    def train_step(self, images, word_targets, class_labels, lengths):
        # Forward, Backward and Optimize
        self.model.zero_grad()
        outputs = self.model(images, word_targets, lengths)
    ...
    def eval_step(self, images, word_targets, class_labels, lengths):
        outputs = self.model(images, word_targets, lengths)
