...
class ImageSentenceClassifier(nn.Module):
    def __init__(self, image_feat_size, word_embed_size, hidden_size, vocab_size, num_classes,
            dropout_prob=0.5):
        super(ImageSentenceClassifier, self).__init__()
        ...
        # Image mapping
        self.im_linear = nn.Linear(image_feat_size, hidden_size)
    ...
    def forward(self, images, captions, lengths):
        ...
        # Image features
        im_map = self.im_linear(images)
        im_txt_fusion = F.normalize(im_map * last_hiddens)
        im_txt_fusion = F.dropout(im_txt_fusion, p=0.8, training=self.training)

        outputs = self.linear(last_hiddens + im_txt_fusion)
        return outputs
